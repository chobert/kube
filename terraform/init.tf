resource "null_resource" "kustomization" {
  ...
  provisioner "file" {
    content = templatefile(
      "${path.module}/templates/ccm.yaml.tpl",
      {
        ...
        default_lb_location = var.location
  ...
  provisioner "file" {
    content = templatefile(
      ...
      {
        channel = local.initial_k3s_channel
    })
  ...
  provisioner "remote-exec" {
    inline = [
      "set -ex",
      "sleep 10",
      "kubectl -n kube-system create secret generic hcloud --from-literal=token=${var.hcloud_token} --from-literal=network=${hcloud_network.main_network.name} --dry-run=client -o yaml | kubectl apply -f -",
      ...
      "curl https://raw.githubusercontent.com/hetznercloud/csi-driver/${local.csi_version}/deploy/kubernetes/hcloud-csi.yml | sed -e 's|k8s.gcr.io|registry.k8s.io|g' > /var/post_install/hcloud-csi.yaml"
    ]
  }
